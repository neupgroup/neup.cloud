...
export function StatusDashboard({ applicationId }: StatusDashboardProps) {
      ...
      case 'running':
        return (
          <div className="flex flex-col">
            ...
            {status.metrics.memory ? <p className="text-xs text-muted-foreground mt-1">Mem: {status.metrics.memory} | CPU: {status.metrics.cpu}</p> : null}
          </div>
        );
      case 'error':
        return (
  ...
  const renderAvailability = () => {
    ...
    if (status?.availability === 'inactive') {
      return (
        <div className="flex flex-col">
          <div className="flex items-center gap-2 text-destructive">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      <Card className={cn("transition-all duration-500", status?.processStatus === 'running' ? "bg-gradient-to-br from-card to-green-500/5 border-green-500/20" : "")}>
      ...
      <Card className={cn(status?.availability === 'active' ? "bg-gradient-to-br from-card to-green-500/5 border-green-500/20" : "")}>
      ...
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={() => void fetchStatus()} disabled={isLoading}>
            Refresh
          </Button>
        </CardHeader>
        <CardContent>
